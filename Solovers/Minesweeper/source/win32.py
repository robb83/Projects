...
class Window:
    ...
    def leftClick(self, position, clicks = 1):
        autoit.MouseClick('left', self.client_position[0] + position[0], self.client_position[1] + position[1], clicks, 1)
        
    def rightClick(self, position, clicks = 1):
        autoit.MouseClick('right', self.client_position[0] + position[0], self.client_position[1] + position[1], clicks, 1)
        
    def click(self, position, clickType = 'left', clicks = 1):
        autoit.MouseClick(clickType, self.client_position[0] + position[0], self.client_position[1] + position[1], clicks, 1)

    def drag(self, position1, position2, clickType = 'left'):
        autoit.MouseMove(self.client_position[0] + position1[0], self.client_position[1] + position1[1], 1)
        autoit.MouseDown(clickType)
        autoit.MouseMove(self.client_position[0] + position2[0], self.client_position[1] + position2[1], 1)
    ...
    def clickDialogButton(self, id, position):
        autoit.MouseClick('left', self.client_position[0] + position[0], self.client_position[1] + position[1], 1, 1)
